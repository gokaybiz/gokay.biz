...
import { useDayjs, useRuntimeConfig } from "#imports";
...
const initializeTime = (timezone: string) =>
  pipe(
    createTimeEffect(timezone),
    Effect.tap((initialTime) => Effect.sync(() => updateTimeData(initialTime))),
    Effect.catchAll((error: TimeUpdateError) =>
      Effect.logError(`Time initialization failed: ${error.message}`).pipe(
        Effect.as(undefined),
      ),
    ),
  );
...
const startTimeUpdates = (timezone: string) =>
  pipe(
    ...
    Effect.catchAll((error: TimeUpdateError) =>
      Effect.logError(`Failed to start time updates: ${error.message}`).pipe(
        Effect.as(undefined),
      ),
    ),
  );
